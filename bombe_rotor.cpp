...
bombe_rotor::bombe_rotor() : enigma_machine(){

}
/* Basic data operations */
inline void bombe_rotor::arrcpy(array<int, 26> &a, const array<int, 26> &b){
    for (int i = 0; i < 26; ++i)
        a[i] = b[i];
}
inline void bombe_rotor::arrcpy(array<bool, 26> &a, const array<bool, 26> &b){
    for (int i = 0; i < 26; ++i)
        a[i] = b[i];
}
/* Basic input operations */
void bombe_rotor::load_text_from_file(string path){
...
inline void bombe_rotor::print_vec(const vector<int> &vec){
    for (int i = 0; i < vec.size(); ++i){
        cout << vec[i] << " ";
    }
    cout << endl;
}
inline void bombe_rotor::print_loop(const vector<int> &loop){
...
inline void bombe_rotor::print_arr(const array<int, 26> &arr){
    for (int i = 0; i < 26;++i){
        cout << setw(2) << arr[i] << " ";
    }
    cout << endl;
}
inline void bombe_rotor::print_plugboard(const array<int, 26> &pb){
    for (int i = 0; i < 26; ++i){
        cout << static_cast<char>(i+65) << static_cast<char>(pb[i] + 65) << " ";
    }
    cout << endl;
}
inline void bombe_rotor::print_plugboard_possibilities(const vector<array<int, 26>> &pb){
    return;
...
/* 
    Find loop functions 
*/
void bombe_rotor::reorganize_loops(){
    /* 
...
int bombe_rotor::is_letter_exists_in_loop(int value, const vector<int> &loop){
...
int bombe_rotor::is_value_exists_full_check(int value, const vector<int> &loop){
    for (int i = 0; i < loop.size(); ++i){
        if(value == loop[i])
            return i;
    }
...
bool bombe_rotor::is_loop_exists(const vector<int> &loop){
...
vector<int> bombe_rotor::sort_loop(const vector<int> &loop){
...
vector<int> bombe_rotor::change_loop_start_letter(const vector<int> &loop, const int &start_value){
    int index = -1;
    for (int i = 0; i < loop.size(); ++i){
...
*/
bool bombe_rotor::test_enigma_init_steps(){
    ...
    */
    ...
    { // iter plugboard, replace empty by A~Z
        ...
        {
            //cout << "spanned empty plugboard[] " << endl;
    ...
    for (int i = 0; i < plugboard_possibilities.size(); ++i){
        ...
        if(bool_pass_all_text_test){
            ...
            arrcpy(plugboard_result, plugboard_possibilities[i]);
...
inline bool bombe_rotor::is_value_in_vec(const vector<int> &vec, const int &num){
    for (int i = 0; i < vec.size(); ++i){
        if(vec[i] == num)
            return true;
    }
...
inline bool bombe_rotor::connect_plugboard(array<int, 26> &arr, const int &a, const int &b){
    if(a!=b){
        if((arr[a] == -1) && (arr[b] == -1)){
            arr[a] = b;
            arr[b] = a;
...
bool bombe_rotor::is_plugboard_valid(int plugboard[26]){
...
void bombe_rotor::add_one_init_step(){
...
void bombe_rotor::add_text_to_bombe_menu(string plain, string cipher){
...
void bombe_rotor::reduce_vec_process(vector<vector<int>> &vec_process){
